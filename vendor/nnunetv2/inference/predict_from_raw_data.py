...
class nnUNetPredictor(object):
    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                # Log the time for this prediction
                time_taken = time.time() - start_time
                prediction_times.append(time_taken)
                print(f"Time taken for {os.path.basename(ofile) if ofile else 'current image'}: {time_taken:.2f} seconds")
        ...
        if total_files > 0:
            average_time = sum(prediction_times) / total_files
            print(f"\nTotal files predicted: {total_files}")
            print(f"Average prediction time per file: {average_time:.2f} seconds")
        else:
            print("\nNo files predicted.")
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # mirror_axes = (0,1,2) # no mirroring model for TTA
